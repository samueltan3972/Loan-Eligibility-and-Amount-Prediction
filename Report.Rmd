...
#### 3.2 Split train set into data (x_train) and label(y_train)
...
#### 3.3 Split test set into data (x_test) and label(y_test)
...
# 5. Regression - 5 Machine Learning algorithms (Linear Regression, Random Forest, XGBoost, Support Vector Regression, K-Nearest Neighbor)
...
theme_classic()+  
ggtitle("Regression models vs RMSE") +
xlab("RMSE") +
ylab("Regression models")

print('Lower RMSE value indicates better fit of the model thus from the bar chart it is observed Random Forest Default model is the best model for this prediction.')
...
scale_colour_gradient2()+
theme_classic()+  
ggtitle("Regression models vs R Squared") +
xlab("R_SQUARED") +
ylab("Regression models")
print('Value of 1 of R Squared indicates perfect fit, from the bar chart it is observed XGBoost-Grid Tuned model has the highest and nearest value to 1.')
```

#### From the bar charts created, it is observed that Random Forest Default and XGBoost Grid Tuned models are the best two models. Thus these two models will be used to plot the Actual vs Predicted values of the models.
```{r}
...
plot(x=xgb_tuned_pred, y= y_test,
     ...
     ylab='Actual Values',
     main='Predicted vs. Actual Values (XGBoost-Grid Tuned)')
abline(a=0, b=1)


# Predicted vs. Actual Values (Random Forest-Default)
plot(x=rf_def_pred, y= y_test,
     xlab='Predicted Values',
     ylab='Actual Values',
     main='Predicted vs. Actual Values (Random Forest-Default)')
abline(a=0, b=1)
```

## Comparison on XGBoost (Fine tuned parameters) and Random Forest (Default parameters)
...
####  Overview on actual vs predictions of new loan amount using Random Forest Default prediction model.
```{r}
data1 <- data.frame(actual= y_test, predicted=rf_def_pred)
head(data1)
```
